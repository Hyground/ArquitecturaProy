...
const FleetManagement: React.FC = () => {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const fetchedFlotas = (fRes.data || []).map((f: any) => ({
        ...f,
        supervisor: allUsers.find(u => u.id === f.supervisorId) || null,
        vehiculos: (f.vehiculos || []).map((v: any) => ({
          ...v,
          conductor: allUsers.find(u => u.id === v.conductorId) || null
        }))
      }));
  ...
  const handleFlotaSubmit = async (e: React.FormEvent) => {
    ...
    const payload = {
      ...
      supervisorId: flotaFormData.supervisorId ? Number(flotaFormData.supervisorId) : null
    };
  ...
  const handleEditFlota = (f: Flota) => {
    ...
    setFlotaFormData({ 
      nombre: f.nombre, 
      supervisorId: f.supervisorId ? String(f.supervisorId) : (f.supervisor ? String(f.supervisor.id) : '') 
    });
  ...
  const handleVehiculoSubmit = async (e: React.FormEvent) => {
    ...
    const payload = {
      ...vehiculoFormData,
      conductorId: vehiculoFormData.conductorId ? Number(vehiculoFormData.conductorId) : null,
  ...
  const handleEditVehiculo = (v: Vehiculo) => {
    ...
    setVehiculoFormData({
      ...
      foto: v.foto || '', conductorId: v.conductorId ? String(v.conductorId) : (v.conductor ? String(v.conductor.id) : ''),
      estado: v.estado
    });
  ...
  return (
    <div className="animate-slide-up">
      ...
      <style>{`
        ...
        .icon-btn { width: 36px; height: 36px; border-radius: 10px; border: 1px solid var(--border); background: rgba(255,255,255,0.03); display: flex; align-items: center; justify-content: center; cursor: pointer; transition: 0.3s; color: var(--text-muted) !important; }
        .icon-btn.primary:hover { color: var(--primary) !important; border-color: var(--primary); background: var(--primary-glow); }
        .icon-btn.danger:hover { color: var(--danger) !important; border-color: var(--danger); background: rgba(239, 68, 68, 0.1); }
        ...
        .flota-stats-container { display: flex; gap: 12px; margin-bottom: 1.5rem; }
        .stat-group { background: rgba(255,255,255,0.03); padding: 1rem; border-radius: 12px; display: flex; gap: 1.5rem; border: 1px solid var(--border); }
        ...
        .flota-footer { display: flex; justify-content: space-between; align-items: center; font-size: 0.85rem; color: var(--primary); font-weight: 700; padding-top: 1rem; border-top: 1px solid var(--border); }
